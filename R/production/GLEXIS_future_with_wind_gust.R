...
library(SPEI, lib.loc="//hdrive/home$/u141/nakhavali/R_libs/library/")
library(sf, lib.loc="//hdrive/home$/u141/nakhavali/R_libs/library/")
library(rnaturalearth, lib.loc="//hdrive/home$/u141/nakhavali/R_libs/library/")
library(rnaturalearthdata, lib.loc="//hdrive/home$/u141/nakhavali/R_libs/library/")
library(lubridate, lib.loc="//hdrive/home$/u141/nakhavali/R_libs/library/")
library(yaml, lib.loc="//hdrive/home$/u141/nakhavali/R_libs/library/")
library(abind, lib.loc="//hdrive/home$/u141/nakhavali/R_libs/library/")
...
library(parallel, lib.loc="//hdrive/home$/u141/nakhavali/R_libs/library/")
...
process_single_year <- function(model, scenario, year, threshold_set = params$thresholds_quantiles, ensemble_index = NULL) {
  ...
  load_thresholds <- function(var) {
    ...
    if(model == "UKESM1-0-LL")     base_pattern <- paste0(model_pattern, "_r1i1p1f2_w5e5")
  ...
  # 14. SUM EVENTS BY MONTH AND CALCULATE MAGNITUDE STATISTICS (OPTIMIZED)
  ...
  # Precompute month indices
  month_indices <- lapply(1:12, function(m) {
    which(month(dates[year_idx]) == m)
  })
  ...
  for (event in names(events)) {
    ...
    # Initialize arrays
    counts <- array(0, dim = c(nlon, nlat, 12))
    
    # Create structure for magnitude statistics (max, min, mean)
    magnitude_stats <- array(NA, dim = c(nlon, nlat, 12, 3),
    ...
    # Process each month
    for (m in 1:12) {
      month_days <- month_indices[[m]]
      if (length(month_days) == 0) next
      
      # Extract month slices
      event_slice <- events[[event]][, , month_days]
      data_slice <- event_data[, , month_days]
      
      # Calculate event counts (fast vectorized)
      valid_cells <- apply(!is.na(event_slice), c(1,2), any)
      counts[, , m] <- rowSums(event_slice, dims = 2, na.rm = TRUE)
      counts[, , m][!valid_cells] <- NA_real_
      
      # Only calculate magnitude for cells with events
      event_cells <- which(counts[, , m] > 0, arr.ind = TRUE)
      
      if (nrow(event_cells) > 0) {
        # Preallocate arrays
        max_temp <- array(NA, dim = c(nlon, nlat))
        min_temp <- array(NA, dim = c(nlon, nlat))
        mean_temp <- array(NA, dim = c(nlon, nlat))
        
        # Process each cell with events
        for (idx in 1:nrow(event_cells)) {
          i <- event_cells[idx, 1]
          j <- event_cells[idx, 2]
          
          # Extract event days for this cell
          event_days <- which(event_slice[i, j, ])
          if (length(event_days) == 0) next
          
          # Get values on event days
          vals <- data_slice[i, j, event_days]
          
          # Calculate statistics
          max_temp[i, j] <- max(vals, na.rm = TRUE)
          min_temp[i, j] <- min(vals, na.rm = TRUE)
          mean_temp[i, j] <- mean(vals, na.rm = TRUE)
        }
        
        # Store results
        magnitude_stats[, , m, "max"] <- max_temp
        magnitude_stats[, , m, "min"] <- min_temp
        magnitude_stats[, , m, "mean"] <- mean_temp
      }
    }
    
    # Store in lists
    monthly_event_counts[[event]] <- counts
    monthly_magnitude_stats[[event]] <- magnitude_stats
  }
  ...
  # Create monthly rasters for each event type (counts only)
  ...
  # Create magnitude rasters for each event type
  monthly_magnitude_rasters <- list()
  for (event in names(monthly_magnitude_stats)) {
    monthly_magnitude_rasters[[event]] <- list(
      max = lapply(1:12, function(m) {
        create_output_raster(monthly_magnitude_stats[[event]][, , m, "max"])
      }),
      min = lapply(1:12, function(m) {
        create_output_raster(monthly_magnitude_stats[[event]][, , m, "min"])
      }),
      mean = lapply(1:12, function(m) {
        create_output_raster(monthly_magnitude_stats[[event]][, , m, "mean"])
      })
    )
  }
...
params$run_type <- "main"
cat("\n===== RUNNING MAIN ANALYSIS =====\n")
for (year in years) {
  result <- tryCatch({
    process_single_year(model, scenario, year, 
                        threshold_set = params$thresholds_quantiles)
  }, error = function(e) {
...
params$run_type <- "ensemble"
cat("\n===== RUNNING ENSEMBLE ANALYSIS =====\n")
for (i in seq_along(params$ensemble_thresholds$tasmax)) {
  cat("\n===== PROCESSING ENSEMBLE SET", i, "=====\n")
  
  # Create threshold set for this ensemble run
